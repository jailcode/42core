...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/29 13:27:01 by pdangwal          #+#    #+#             */
/*   Updated: 2025/09/29 14:08:05 by pdangwal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_conn(t_map *map, t_game *data)
{
	...
	data->win = mlx_new_window(data->mlx, (xsize) * 32, 
			(ysize) * 32, "game :D");
	if (!data->win)
	{
...
{
	map->sprites[0] = mlx_xpm_file_to_image(data->mlx, "src/assets/2.xpm",
			&(map->s), &(map->s));
	map->sprites[1] = mlx_xpm_file_to_image(data->mlx, "src/assets/4.xpm",
			&(map->s), &(map->s));
	map->sprites[2] = mlx_xpm_file_to_image(data->mlx, "src/assets/3.xpm",
			&(map->s), &(map->s));
